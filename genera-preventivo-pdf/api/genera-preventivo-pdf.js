...
const headerBase64Data = require('../assets/header-base64');

function getHeaderImage() {
  return 'data:image/png;base64,' + headerBase64Data;
}
...
module.exports = async (req, res) => {
  if (req.method !== 'POST') {
    res.status(200).json({ error: 'Metodo non consentito, usare POST' });
    return;
  }
  ...
  try {
    ...
    if (!payload || !payload.numeroPreventivo || !Array.isArray(payload.righe)) {
      res.status(200).json({ error: 'Payload non valido: servono numeroPreventivo e righe[]', payloadRicevuto: payload });
      return;
    }
    ...
    pdfDoc.on('error', function (err) {
      res.status(200).json({ error: 'Errore generazione PDF (stream): ' + err.message, stack: err.stack });
    });
    pdfDoc.end();
  } catch (err) {
    res.status(200).json({ error: 'Errore generazione PDF: ' + err.message, stack: err.stack });
  }
};
